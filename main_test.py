...
#                     help='interger list')
# argparse.ArgumentParser(description='Add some integers.')
# parser.add_argument('integers', metavar='N', type=int, nargs='+',
#                     help='interger list')
#
...
def syntax_highlight_space(input):
   stripped = input.rstrip()
   return stripped + Back.LIGHTBLUE_EX + " " * (len(input) - len(stripped)) + u"\u001b[0m"

def _syntax_highligh(input):
    return
...
def command_line():
    ...
    while True:  # loop for each line
        ...
        while True:  # loop for each character
            ...
            sys.stdout.write(syntax_highlight_space(input))
            sys.stdout.write(u"\u001b[1000D")
            if index > 0:
